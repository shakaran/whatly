...
- **Test coverage raised to 85%.** New headless suites drive the parts the pure-logic tests could not reach: the network clients (Ollama check/pull, the translator and the AI assistant) against a stand-in HTTP server; the local API server end to end over loopback (auth, routing, the `/send` and `/webhook` endpoints); the widgets offscreen — the command palette's filtering and keyboard handling, the account tab bar as both a click target and a drop target, and the dictionary-row delegate's painting across its download/downloading/failed/bundled states; the tar-based profile export/import; the drop reader and drop resolver; scheduled-message due-checking and recurrence; session-backup startup recovery; and the chat-export formatting. Several modules go from a third or a quarter covered to 90%+.
