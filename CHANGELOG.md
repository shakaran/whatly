...
- **Test coverage raised to 80%.** New headless suites drive the parts the pure-logic tests could not reach: the network clients (Ollama, the translator and the AI assistant) against a stand-in HTTP server; the local API server end to end over loopback (auth, routing, the `/send` and `/webhook` endpoints); and the widgets offscreen — the command palette's filtering and keyboard handling, the account tab bar, and the dictionary-row delegate's painting across its download/downloading/failed/bundled states. `commandpalette` and `dictionaryrows` go from a quarter covered to 95%.
