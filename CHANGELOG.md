...
- **Swedish (sv_SE) is available as an interface language,** the 26th. It is a full translation of the English source (787 strings), leaving only untranslatable placeholders, URLs and unit labels to fall back to English.
- **Galician (gl_ES) is available as an interface language,** the 27th. It is a full translation of the English source (787 strings), leaving only untranslatable placeholders, URLs and unit labels to fall back to English.
- **Danish (da_DK) is available as an interface language,** the 28th. It is a full translation of the English source (787 strings), leaving only untranslatable placeholders, URLs and unit labels to fall back to English.
- **Norwegian Bokmål (nb_NO) is available as an interface language,** the 29th. It is a full translation of the English source (787 strings), leaving only untranslatable placeholders, URLs and unit labels to fall back to English.
- **Greek (el_GR) is available as an interface language,** the 30th. It is a full translation of the English source (787 strings), leaving only untranslatable placeholders, URLs and unit labels to fall back to English.
...
- **The Flatpak build moves off the end-of-life KDE 6.9 runtime (#111).** It now targets the KDE 6.10 runtime (Qt 6.10), which matches Whatly's default Qt floor, so the previous `-DQT_VERSION_MINOR=9` override is dropped. Both the local and Flathub manifests are updated.
- **"Find in chats" is now reachable from the command palette.** The `Ctrl+F` chat-list search was already there; it now also appears in the `Ctrl+K` command palette, so it can be reached by name like any other command.
- **A global "boss key" hides every window at once.** `Ctrl+Alt+H` hides all Whatly windows immediately and suppresses message popups while hidden, so nothing surfaces after the hide; pressing it again (or the existing `Ctrl+Alt+W`, or the tray) brings them back and re-enables popups. Unread badges keep updating meanwhile. Where the desktop does not allow an app to register global shortcuts, it registers nothing, the same as the existing raise and quick-compose ones.
- **The restart no longer risks wedging the new instance at 100% CPU (#98).** "Restart now" (after a setting that needs a restart, or a self-update) spawned the new instance by `fork()`-ing the running process, but a QtWebEngine (Chromium) process is not fork-safe: on some setups, an AppImage on Wayland among them, the new instance could spin on a single core indefinitely and had to be killed. It now uses `posix_spawn` instead, which starts a fresh process that execs at once without cloning Chromium's threads and locks, keeping the same new-session detach and the stdout/stderr the log needs (#62). A libc too old for `posix_spawn`'s session and descriptor controls keeps the previous fork-based path.
...
- **A fresh install fetches the system language's dictionary on first run, and bundles none (#110).** The packages shipped one dictionary, `en_US`, so spell-check was never silently dead, but that is the right default for nobody in particular: a non-English user got an English dictionary they never chose and still had to download their own. The build now bundles no dictionary at all (`WHATLY_BUNDLE_DICTIONARIES` defaults to `NONE`), and on first run, when nothing is installed, Whatly fetches the one matching the system locale from the `dictionaries` release, by full name first (`es_ES`) then by language (an `es_AR` system still gets Spanish). The result is a smaller install and a better outcome than `en_US` for everyone whose system is not American English. The stop conditions are the real design, so it never becomes a nuisance: a dictionary removed in Settings records a deliberate opt-out and is never pulled back in; a system language the manifest does not carry (an `eo` system today) is remembered against the manifest's own identity and re-checked only when the manifest changes, logged once rather than every launch; and the fetch is one attempt at startup with a short bounded backoff for a connection still coming up, then nothing until the next launch, never a timer that polls for ever. Once a dictionary exists, or a choice has been recorded, none of this acts again.
- **WhatsApp's own “Refresh to update” notice no longer wrecks the collapsed chat list.** When WhatsApp has a new version waiting it puts a notice directly above the chat list: an icon, a heading, a sentence and a Refresh button. A chat row keeps its name and message on one line, so the collapsed strip clips it and the picture is what survives — but a notice's sentence wraps instead, and a 97px column turns it into a ladder of single letters running the height of the strip, with its button pushed off the side where it cannot be reached at all. The notice is now handled as though it were the topmost chat: its icon stays where an avatar sits, its words are clipped exactly as a row's are, and hovering it shows the whole notice in the same preview panel the chats already use. It is found by the name WhatsApp itself gives it — `data-testid="chat-butterbar"`, WhatsApp's own word for this class of bar — rather than by the class names sitting beside it on the same element, which are generated afresh with every deploy and are worth nothing to match on; the position it occupies, immediately above the list, is kept behind that as a fallback should the name ever change. That element stays in the page while empty once the update has been applied, so an empty one is deliberately not treated as a notice: what is looked for is an icon, something to click, and a sentence rather than a word. Clicking the collapsed notice carries the click to the notice's own button, and where there is no button to carry it to the click falls through and opens the list back up instead, which is what a click anywhere else above the list already does. Where nothing matches, the strip behaves exactly as it did before.
- **The spell-check dictionary licenses are documented.** Now that the packages bundle no dictionary and the copyleft ones are conveyed purely as separate downloadable data assets, `src/dictionaries/LICENSES.md` lists each language's license and its provenance (the LibreOffice dictionaries project), and the full GNU texts in use (GPL-2.0, GPL-3.0, LGPL-2.1, LGPL-3.0, AGPL-3.0) are kept under `src/dictionaries/licenses/`.
...
- **With the custom window frame on, WhatsApp Web no longer stops loading older messages (#104).** Switching chats and back with the custom (frameless) frame enabled left a thread unable to fetch older messages until a reload — while a real browser was fine. A frameless window can read to the compositor as occluded or backgrounded, and Chromium then throttles the renderer and its timers, which is exactly what stalls WhatsApp's multi-device message sync. Whatly now turns that background throttling off when the custom frame is in use (`--disable-backgrounding-occluded-windows` and friends), so the page keeps syncing whatever the frame does; the default (native-frame) behaviour is unchanged. Thanks to @skeezmoe for pinning it to the custom-frame option.
- **The "no H.264 codecs" notice now also explains why calls are greyed out (#97).** WhatsApp Web's voice and video calls need the same proprietary H.264 codec that the codec-free portable builds omit, so on those builds the call buttons sit greyed with an "unsupported browser" message and no hint as to why. The one-time notice that already fires when the engine can't do H.264/MP4 now adds a line saying voice and video calls are unavailable for the same reason — so the greyed buttons are explained rather than mysterious. A codec-enabled Qt WebEngine (the Flatpak, or a source build with the proprietary codecs) restores them.
- **The app icon is sharp in the task bar again, including on Flatpak (#105).** The window icon was looked up in the icon theme under the name `whatly`, but every installed icon — all the hicolor PNG sizes and the scalable SVG — is named `net.shakaran.whatly` (the app id). So the lookup always missed and fell through to a single bundled 64px raster, which a panel or task bar then blurred when it drew the icon larger. It now asks the theme for `net.shakaran.whatly`, so the full size set (and the SVG) is used and the icon stays crisp at any size; the built-in fallback, for a run from the build tree, is now a multi-size icon rather than the lone 64px one.
- **Settings can be opened without a system tray (#103).** The window has no menu bar or chrome of its own, so the tray icon's menu was the only *visible* way into Settings — and where there is no system tray (some Flatpak and Wayland setups) that left only the undiscoverable Ctrl+P. A small gear now appears in the corner of the page in exactly that case — no tray available — and opens Settings; it is not shown when a tray is present, so the usual clean look is unchanged. (Ctrl+P still works everywhere, and Ctrl+K opens the command palette.)
- **A switch to collect scroll diagnostics for a bug report.** "Scrolling feels janky" is hard to act on without numbers, and impressions are wrong more often than measurements. The Performance settings now carry a **Collect scroll diagnostics** switch: while it is on, a passive probe records how the chat actually scrolls — wheel-to-pixel latency, distance per notch, direction reversals and a frame-gap histogram — into the page, and **Copy diagnostics** puts what it has gathered on the clipboard and into the log, ready to paste into an issue. It is off by default, injects nothing and costs nothing while off, and the probe is the very file its test harnesses check (`debug/`), so what a reporter sends is what was measured.
